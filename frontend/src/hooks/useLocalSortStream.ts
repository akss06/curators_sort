...
export interface LocalSortParams {
  folderPath: string
  priorities: string[]
  limit: number
...
interface UseLocalSortStreamReturn {
  status: SortStatus
  progress: SortProgress | null
  logs: LogEntry[]
  error: string | null
  startSort: (params: LocalSortParams) => void
  reset: () => void
}

export function useLocalSortStream(): UseLocalSortStreamReturn {
  ...
  const startSort = useCallback(async (params: LocalSortParams) => {
    esRef.current?.close()
    setStatus('starting')
    setProgress(null)
    setLogs([])
    ...
    try {
      const res = await fetch('/api/local-sort/start', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          limit: params.limit,
          allow_new_folders: params.allowNewFolders,
      ...
      setStatus('running')
      const es = new EventSource(`/api/local-sort/stream/${job_id}`)
      esRef.current = es
      resetStreamTimeout()
